...
public class BearsArrangement {
    ...
    {
        // init rules layout
        LinearLayout linearRuleOne = (LinearLayout) linearGameRules.getChildAt(0);
        LinearLayout linearRuleTwo = (LinearLayout) linearGameRules.getChildAt(1);
    ...
    public boolean check_arrangement(ArrayList<Integer> userBearsGuess)
    {
        ...
        for(int i = 0; i < userBearsGuess.size() - 1; i++)
        {
            ...
            if(userBearsGuess.get(i) == this.getGame_rules()[0])
            {
                // check if bear [i] sit next to bear [i + 1]
                if(userBearsGuess.get(i + 1) == this.getGame_rules()[1])
                {
                    return false;
                }
            }
            ...
            // check the current place with the first rule
            if(userBearsGuess.get(i) == this.getGame_rules()[1])
            {
                // check if bear [i] sit next to bear [i + 1]
                if(userBearsGuess.get(i + 1) == this.getGame_rules()[0])
                {
                    return false;
                }
            }
            ...
            if(this.getGame_rules().length > 2){
                if(userBearsGuess.get(i) == this.getGame_rules()[2])
                {
                    if(userBearsGuess.get(i + 1) == this.getGame_rules()[3])
                    {
                        return false;
                    }
                }

                if(userBearsGuess.get(i) == this.getGame_rules()[3])
                {
                    if(userBearsGuess.get(i + 1) == this.getGame_rules()[2])
                    {
                        return false;
                    }
                }
